...
for (const line of logLines) {
  ...
  if (parts.length >= 15 && partsIdAddress == logTypeName) {
    ...
    //   }
     ...
     const file_Path = path.join(__dirname, 'public', 'data.json');
     console.log(file_Path);
     

     try {
         // Write (or overwrite) the data to data.json
         fs.writeFileSync(file_Path, JSON.stringify(parsedLogs, null, 2), 'utf8');
         res.status(200).json({
           message: `${parsedLogs.length} Data saved successfully`,
          // data: parsedLogs 
          });
     } catch (error) {
